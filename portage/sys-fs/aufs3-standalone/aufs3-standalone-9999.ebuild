...
pkg_setup() {
	...
	get_version
	kernel_is lt 3 0 && die "kernel too old--try aufs2 instead"
	...
	linux-mod_pkg_setup
	if ! ( patch -p1 --dry-run --force -R -d ${KV_DIR} < "${FILESDIR}"/aufs${VERSION}-standalone.patch >/dev/null && \
		patch -p1 --dry-run --force -R -d ${KV_DIR} < "${FILESDIR}"/aufs${VERSION}-proc_map.patch >/dev/null && \
		patch -p1 --dry-run --force -R -d ${KV_DIR} < "${FILESDIR}"/aufs${VERSION}-base.patch >/dev/null ); then
		if use kernel-patch; then
			cd ${KV_DIR}
			ewarn "Patching your kernel..."
			patch --no-backup-if-mismatch --force -p1 -R -d ${KV_DIR} < "${FILESDIR}"/aufs${VERSION}-standalone.patch >/dev/null
			patch --no-backup-if-mismatch --force -p1 -R -d ${KV_DIR} <	"${FILESDIR}"/aufs${VERSION}-proc_map.patch >/dev/null
			patch --no-backup-if-mismatch --force -p1 -R -d ${KV_DIR} < "${FILESDIR}"/aufs${VERSION}-base.patch >/dev/null
			epatch "${FILESDIR}"/aufs${VERSION}-{base,proc_map,standalone}.patch
			ewarn "You need to compile your kernel with the applied patch"
			ewarn "to be able to load and use the aufs kernel module"
		else
			eerror "You need to apply a patch to your kernel to compile and run the aufs2 module"
			eerror "Either enable the kernel-patch useflag to do it with this ebuild"
			eerror "or apply 'patch -p1 < ${FILESDIR}/aufs${VERSION}-base.patch' and"
			eerror "'patch -p1 < ${FILESDIR}/aufs${VERSION}-standalone.patch' by hand"
